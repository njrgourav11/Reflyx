  ...
  return (
    <footer className="py-10 border-t border-slate-200/60 dark:border-slate-800 text-sm mt-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-slate-500">© {new Date().getFullYear()} Reflyx • MIT License • Developed by <Link href="https://www.gourav.bio" className="underline">Gourav</Link></p>
        <nav className="flex items-center gap-4">
          ...
          <Link href="https://github.com/njrgourav11/Reflyx" target="_blank" rel="noopener" className="nav-link">Repository</Link>
          <Link href="/docs" className="nav-link">Docs</Link>
          <Link href="mailto:njrgourav@gmail.com" className="nav-link">Contact</Link>
        </nav>
      </div>
    </footer>
  );
...
function SocialIcon({ label, href, svg }: { label: string; href: string; svg: React.ReactNode }){
  return (
    <Link href={href} aria-label={label} className="icon-btn" target="_blank" rel="noopener">
      {svg}
    </Link>
  );
}
